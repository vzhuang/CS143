...
Packet * Link::transmit_packet() {
	// Sanity check
	if(data_buffer.empty()) {
			printf("Failure. Attempted to transmit a packet on a link with an empty buffer. Global Time: %f, t_free: %f\nExiting. \n",
			global_time * 1000.0, t_free * 1000.0);
			exit(-1);
	}

	if(is_free == false) {
			printf("Failure. Link was not free but a transmit was attempted. Global Time: %f, t_free: %f\nExiting. \n",
			global_time * 1000.0, t_free * 1000.0);
			exit(-1);
	}
	...
	{
			printf("Failure. Global Time: %f, t_free: %f\nExiting. \n",
			global_time * 1000.0, t_free * 1000.0);
			exit(-1);
	}
